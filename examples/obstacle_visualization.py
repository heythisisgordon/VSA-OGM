"""
Visualization example for Sequential VSA-OGM using obstacle map data.

This example demonstrates the visualization capabilities of the Sequential VSA-OGM system
using the obstacle map data.
"""
...
def main():
    ...
    print("Sequential VSA-OGM Visualization Example (Obstacle Map)")
    
    # Load obstacle map point cloud
    print("Loading obstacle map point cloud...")
    obstacle_map = np.load("data/obstacle_map.npy")
    
    # Extract points from obstacle map
    # Assuming the obstacle map is a 2D grid where non-zero values represent obstacles
    # Convert to point cloud format
    print("Converting obstacle map to point cloud...")
    y_indices, x_indices = np.where(obstacle_map > 0)
    ...
    config_dict = {
        ...
        "sequential": {
            "sample_resolution": 2.0,  # Larger resolution for faster processing
            "sensor_range": 20.0,      # Larger range for better coverage
        },
        "entropy": {
            "disk_radius": 3,
    ...
    fig = plot_point_cloud(
        points, 
        labels, 
        title="Obstacle Map Point Cloud",
        figsize=(10, 8)
    )
    save_figure(fig, "examples/output/viz_obstacle_point_cloud.png")
    plt.close(fig)
    ...
    save_figure(fig, "examples/output/viz_obstacle_quadrants.png")
    plt.close(fig)
    ...
    save_figure(fig, "examples/output/viz_obstacle_sample_positions.png")
    plt.close(fig)
    ...
    # Save figure
    save_figure(fig, "examples/output/viz_obstacle_combined.png")
    plt.close(fig)
    ...
    # Save figure
    save_figure(fig, "examples/output/viz_obstacle_results_grid.png")
    plt.close(fig)
    ...
    save_figure(fig, "examples/output/viz_obstacle_combined_results.png")
    plt.close(fig)
